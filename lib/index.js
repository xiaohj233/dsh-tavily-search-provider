...
import { createCrawlTool, createExtractTool, createMapTool, createResearchGetTool, createResearchTool } from "./tools.mjs";
...
export class TavilySearchProvider {
	...
	async search(request, signal) {
		...
		let response;
		try {
			response = await tavilyFetch(TAVILY_SEARCH_ENDPOINT, {
				method: "POST",
			...
			throw new WebError(`Tavily search request failed: ${String(error)}${tavilyNetworkHint(error)}`, "WEB_PROVIDER_ERROR", { cause: error });
		}
		if (!response.ok) {
...
/** Current LAN IPv4 literals so the LAN page's Host header is not rejected. */
...
/** Whether a request may reach this plugin's /api route. */
...
export function apply(ctx, config) {
	...
	// The same browser-trust fence dsh-client-connection applies to /api.
	...
	ctx.tools.register(
		defineTool({
			...
			parameters: {
				...
				chunks_per_source: {
					type: "integer",
					description: "Chunks per source, 1-5, only used with advanced/fast search_depth (each chunk max 500 chars).",
				},
				start_date: {
					type: "string",
				...
				country: {
					type: "string",
					description: "Boost results from a specific country (e.g. 'united states', 'china').",
				},
				include_images: {
					type: "boolean",
					description: "Include image results (array of image URLs in the response).",
				},
				include_image_descriptions: {
					type: "boolean",
					description: "Include AI-generated descriptions for image results (requires include_images).",
				},
				include_favicon: {
					type: "boolean",
					description: "Include the favicon URL per result.",
				},
				auto_parameters: {
					type: "boolean",
					description: "Let Tavily auto-configure search parameters from the query intent (may raise search_depth to advanced = 2 credits).",
				},
				include_usage: {
					type: "boolean",
			...
			async execute(args, exec) {
				...
				let res;
				try {
					res = await tavilyFetch(TAVILY_SEARCH_ENDPOINT, {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify(body),
						signal: exec.signal,
					...
					if (exec.signal?.aborted === true) throw new Error("tavily_search: request aborted");
					throw new Error(`tavily_search: request failed: ${String(error)}${tavilyNetworkHint(error)}`);
				}
				if (!res.ok) {
	...
	// ---- Tavily API suite (extract / map / crawl / deep research) ----------
	// Same registration policy as tavily_search: independent of the official
	// switch, key resolved per call, clear errors when missing.
	for (const tool of [
		createExtractTool(ctx),
		createMapTool(ctx),
		createCrawlTool(ctx),
		createResearchTool(ctx),
		createResearchGetTool(ctx),
	]) {
		ctx.tools.register(tool);
	}

	ctx.logger?.info("dsh-tavily-search-provider: tavily_search tool registered");
	ctx.logger?.info("dsh-tavily-search-provider: Tavily API suite registered (tavily_extract/tavily_map/tavily_crawl/tavily_research/tavily_research_get)");
}
